...
@Component({
  selector: 'app-tamu-keluar',
  templateUrl: './tamu-keluar.page.html',
  styleUrls: ['./tamu-keluar.page.scss'],
  standalone: false,
})
export class TamuKeluarPage implements OnInit, OnDestroy {
  ...
  async processQRCode(qrData: string) {
    try {
      ...
      // Proses tamu keluar dengan ID kunjungan dari QR code
      await this.processTamuKeluar(qrObject.kunjungan_id);
  ...
  async processManualInput() {
    ...
    await this.processTamuKeluar(this.manualKunjunganId);
  }
      ...
      this.kunjunganService.tamuKeluar(kunjunganId).subscribe({
        next: async (response) => {
          ...
          if (response.success) {
            ...
            this.presentToast('Tamu berhasil diproses keluar', 'success');
            
            // Navigasi ke halaman scan berhasil
            this.router.navigate(['/satpam/beranda/kunjungantamu/scan-berhasil', kunjunganId, 'keluar']);
          } else {
            this.presentToast('Gagal memproses tamu keluar: ' + response.message, 'danger');
          }
        },
        error: (error) => {
          loading.dismiss();
          this.isLoading = false;
          console.error('Error processing tamu keluar:', error);
          this.presentToast('Terjadi kesalahan saat memproses tamu keluar', 'danger');
        }
      });
    } catch (error) {
      this.isLoading = false;
      console.error('Error in processTamuKeluar:', error);
      this.presentToast('Terjadi kesalahan saat memproses tamu keluar', 'danger');
    }
  }
