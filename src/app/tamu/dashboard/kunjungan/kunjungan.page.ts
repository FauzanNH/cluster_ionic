...
export class KunjunganPage implements OnInit {
  ...
  ionViewWillEnter() {
    // Periksa apakah perlu refresh data dari tambah kunjungan
    const needRefresh = localStorage.getItem('refresh_kunjungan');
    if (needRefresh === 'true') {
      // Hapus flag refresh dan reload data
      localStorage.removeItem('refresh_kunjungan');
      this.loadKunjunganData();
    }
  }
  ...
  async loadKunjunganData() {
    ...
    try {
      // Ambil tamu_id dari localStorage
      const tamuData = localStorage.getItem('tamu_data');
      
      if (!tamuData) {
        this.presentToast('Data tamu tidak ditemukan, silakan login kembali', 'danger');
        this.router.navigate(['/login-tamu']);
        return;
      }
      
      const tamu = JSON.parse(tamuData);
      const tamuId = tamu.tamu_id;
      
      console.log('Tamu ID:', tamuId); // Debug log
      ...
      this.kunjunganService.getKunjunganByTamu(tamuId).subscribe({
        next: (response) => {
          console.log('API Response:', response); // Debug log
          if (response.success) {
            // Transformasi data dari API ke format yang digunakan di tampilan
            this.allVisits = response.data.map(item => this.transformKunjunganData(item));
  ...
  transformKunjunganData(kunjungan: Kunjungan): KunjunganTamu {
    // Format tanggal dari ISO string ke format DD/MM/YYYY menggunakan zona waktu lokal perangkat
    const createdDate = new Date(kunjungan.created_at);
    const tanggal = `${createdDate.getDate()}/${createdDate.getMonth() + 1}/${createdDate.getFullYear()}`;
    
    // Simpan waktu dalam format ISO untuk dikonversi ke zona waktu lokal perangkat di tampilan
    let waktuMasuk = kunjungan.waktu_masuk || '-';
    let waktuKeluar = kunjungan.waktu_keluar || undefined;
    
    // Ambil blok dari rumah_id atau dari data rumah jika ada
    let blok = kunjungan.rumah_id;
    if (kunjungan.rumah && kunjungan.rumah.blok_rt) {
      blok = kunjungan.rumah.blok_rt;
    }
    
    // Pastikan semua properti memiliki nilai default yang valid
    return {
      id_kunjungan: kunjungan.kunjungan_id || '',
      tanggal: tanggal || '',
      tujuan: kunjungan.tujuan_kunjungan || '',
      status: kunjungan.status_kunjungan || 'Menunggu Menuju Cluster',
      blok: blok || '',
  ...
  filterVisits() {
    ...
    if (this.searchTerm) {
      filtered = filtered.filter(visit => 
        ...
        visit.blok.toLowerCase().includes(this.searchTerm) ||
        visit.tujuan.toLowerCase().includes(this.searchTerm)
      );
    }
  ...
  onKunjunganBaru() {
    // Navigasi ke halaman form kunjungan baru
    this.router.navigate(['/tamu/dashboard/kunjungan/tambah-kunjungan']);
  }

  navigateToDetail(kunjunganId: string) {
    this.router.navigate(['/tamu/dashboard/kunjungan/view', kunjunganId]);
  }

  // Metode untuk format tanggal (DD/MM/YYYY) dengan zona waktu perangkat
  formatDateOnly(dateString: string | undefined): string {
    if (!dateString) return '';
    
    try {
      // Jika dateString adalah format DD/MM/YYYY, konversi ke Date
      ...
      // Jika dateString adalah ISO string, konversi ke zona waktu lokal
      const date = new Date(dateString);
      if (isNaN(date.getTime())) {
        console.warn('Invalid date format for formatDateOnly:', dateString);
        return dateString;
      }
      
      // Tambahkan nama hari ke format tanggal
      const days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
      const day = days[date.getDay()];
      
      // Format tanggal: "Senin, DD/MM/YYYY" dalam zona waktu lokal
      return `${day}, ${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()}`;
    } catch (error) {
  ...
  // Metode untuk format waktu saja (HH:MM) dengan zona waktu perangkat
  formatTimeOnly(dateTimeString: string | undefined): string {
    if (!dateTimeString || dateTimeString === '-') return '-';
    
    try {
      // Jika sudah dalam format HH:MM, kembalikan langsung
      if (/^\d{1,2}:\d{2}$/.test(dateTimeString)) {
        return dateTimeString;
      }
      
      // Konversi timestamp ISO ke Date object dengan zona waktu perangkat
      const date = new Date(dateTimeString);
      if (isNaN(date.getTime())) {
        console.warn('Invalid date format for formatTimeOnly:', dateTimeString);
        return dateTimeString;
      }
      
      // Format waktu: "08:30" menggunakan zona waktu lokal perangkat
  ...
  // Metode untuk format tanggal dan waktu lengkap dengan zona waktu perangkat
  formatFullDateTime(dateTimeString: string | undefined): string {
    if (!dateTimeString || dateTimeString === '-') return '-';
    
    try {
      const date = new Date(dateTimeString);
      if (isNaN(date.getTime())) {
        console.warn('Invalid date format for formatFullDateTime:', dateTimeString);
        return dateTimeString;
      }
      
      // Format tanggal dan waktu lengkap: "10 Juni 2025, 08:30" menggunakan zona waktu lokal
